...
def pca(X=np.array([]), no_dims=50):
	...
	# Now W is eigenvector (no_dims, 784)
...
def tsne(X=np.array([]), no_dims=2, initial_dims=50, perplexity=30.0):
	...
	for iter in range(max_iter):
		...
		if (iter + 1) % 10 == 0:
			...
			pylab.savefig("tsne_perplexity=20-" + str((iter+1)/10) + ".jpg")
...
if __name__ == '__main__':
	...
	Y = tsne(X, 2, 50, 20.0)
	pylab.scatter(Y[:, 0], Y[:, 1], 20, labels)
	pylab.savefig("tsne_perplexity=20.png")
	pylab.show()
	...
	plt.savefig("tsne_dist_perplexity=20.png")
	plt.show()